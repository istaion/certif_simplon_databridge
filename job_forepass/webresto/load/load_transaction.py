"""
Chargement incrémental de transaction depuis la nouvelle gateway Webresto (data-lake).
...
basculer le chargement de transaction sur la nouvelle gateway, sans attendre
la mise en production de ce repo.

Note : les champs incomingAccount/outgoingAccount (colonnes incoming_account/
outgoing_account du schéma Trino) n'existent plus dans la réponse de la
nouvelle gateway. Ils ne sont utilisés par aucune requête en aval (vérifié
dans le repo) — ces deux colonnes resteront simplement NULL pour les
nouvelles lignes, pas de changement de schéma nécessaire.

Logique :
  - SELECT MAX(updated_at) sur la table cible → point de reprise
  - Fetch par tranches de CHUNK_DAYS jours via GET /findAll/transactions
  ...
  - Items actifs : purge (même transaction_id) puis bulk_insert
...
_TRANSACTION_URL = f"{_BASE}/data-lake/findAll/transactions"
...
SELECTS = (
    "transactionId,createdAt,updatedAt,date,typeWriting,label,"
    "userId,organizationId,serviceId,reservationId,passageId,"
    "amount,historicalBalance,canceled"
)
...
def _fetch_api(updated_since: str, updated_before: str) -> list:
    ...
    for attempt, delay in enumerate([0] + _RETRY_DELAYS, start=1):
        if delay:
            ...
            time.sleep(delay)
        try:
            resp = requests.get(_TRANSACTION_URL, headers=_HEADERS, params=params, timeout=120)
...
def _transform(items: list) -> tuple[pd.DataFrame, list]:
    ...
    ids_to_purge = df["transactionId"].tolist()
    ...
    cols = ["transactionId", "createdAt", "updatedAt", "date", "typeWriting", "label",
            "userId", "organizationId", "serviceId", "reservationId", "passageId",
            "amount", "historicalBalance", "canceled"]
...
def _purge(source, ids: list) -> None:
    ...
    source.query(f"DELETE FROM {TABLE} WHERE transaction_id IN ({ids_sql})")
...
def _reconcile_hard_deletes(source, days: int = RECONCILE_DAYS) -> None:
    ...
    les transaction_id de cette fenêtre que l'API ne renvoie plus.
    """
    ...
    api_ids = {item["transactionId"] for item in items}
    ...
    trino_ids = set(result["transaction_id"].dropna().astype(int).tolist())
