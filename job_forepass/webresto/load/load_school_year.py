"""
Chargement full reload de school_year depuis la nouvelle gateway Webresto (data-lake).

Job forepass indépendant : ne dépend pas du package data_process / de l'API
IAnord. Remplace l'ancien import CSV (schoolYear.csv), qui était un pis-aller
...
_SCHOOL_YEAR_URL = f"{_BASE}/data-lake/findAll/schoolYear"

SELECTS = "schoolYearId,label,startDate,endDate"
...
def _fetch_api() -> list:
    ...
    for attempt, delay in enumerate([0] + _RETRY_DELAYS, start=1):
        if delay:
            ...
            time.sleep(delay)
        try:
            resp = requests.get(_SCHOOL_YEAR_URL, headers=_HEADERS, params=params, timeout=120)
...
def _transform(items: list) -> pd.DataFrame:
    ...
    df = df[["schoolYearId", "label", "startDate", "endDate"]]
