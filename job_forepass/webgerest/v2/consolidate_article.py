"""
Consolidation des tables article V2 vers wg_test_centre_article (schéma V1).
...
TARGET_TABLE   = "wg_test_centre_article"
...
_V2_COLS = (
    "arcleunik, codart, libart, codfamart, sfaart, usart, dpaart, pmpart, "
    "qtethe, qtecom, dcreart, slqte, codstk, codtva, conditionne, suivi, "
    "percde, delai, reglecalc, catol, pdsbrut, glucides, lipides, protides, "
    "energie, qtecomus, reglearr, uniteft, seuilmaxi, nature, pdsvol, statut, "
    "datmod, id_marche, id_lots, bio, designation_externe, allergenes, "
    "codarticle, usart_vers_ufam, logincrea"
)
...
def _sub_select(identifier: str, source_table: str, statut: int) -> str:
    return (
        f"SELECT\n"
        f"    CAST('{identifier}' || '_' || CAST(arcleunik AS VARCHAR) AS VARCHAR) AS pk,\n"
...
def customfunc(event):
    logger.info("job : consolidate_article V2 → V1")
    ...
    for grp in LOGIN_GROUPS:
        ...
        if statut == 2:
            for site in login_map.get(grp, []):
                entries.append((site, f"{SERVER_PREFIX}{_safe_id(site)}_article", statut))
        else:
            entries.append((grp, f"{SERVER_PREFIX}{_safe_id(grp)}_article", statut))
    ...
    logger.info(f"consolidate_article terminé — {n_chunks} snapshot(s) créé(s)")
