"""
Chargement incrémental de mvtart en architecture V2.

Pour chaque table cible {PREFIX_TABLE}{safe_id(identifier)}_mvtart :
  - SELECT MAX(dtemvt) → from_date (ou DEFAULT_START_DATE si vide)
  - Fetch API avec from_date
  - DELETE WHERE dtemvt >= min(données reçues)
  - bulk_insert
...
# même si MAX(dtemvt) est dans le futur (dates de planification, anomalies).
...
# ── Colonnes finales V2 (ordre du schéma, sans login_site / pk / descfic_statut) ──
...
def _transform(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in ["qteart", "poremise", "prixht", "taux_tva", "pmpart", "uatous",
                "qteusart", "pcb", "totht", "totttc", "pmpart_ttc", "qtefac",
                "pufac", "uatoufac", "qtef", "puf", "stockavant", "pmp_avt"]:
        if col in df.columns:
            df[col] = pd.to_numeric(df[col], errors="coerce").astype("float64")

    if "dtemvt" in df.columns:
        df["dtemvt"] = pd.to_datetime(df["dtemvt"], errors="coerce")

    for col in ["dlc", "dteimp"]:
        if col in df.columns:
...
# ── Récupérer la dernière date par table cible ────────────────────────────────

def _get_last_date(source, target_table: str) -> str:
    cutoff = (datetime.now() - timedelta(days=LOOKBACK_DAYS)).strftime("%Y-%m-%d")
    try:
        result = source.query(f"SELECT MAX(dtemvt) AS last_date FROM {target_table}")
        ...
        logger.warning(f"  [{target_table}] impossible de lire MAX(dtemvt): {e}")
...
def customfunc(event):
    logger.info("job : load_mvtart V2")
    ...
    # 2. descfic : statut mvtart par groupe
    ...
    # 3. Construire les tâches (identifier, target_table)
    ...
    for grp in LOGIN_GROUPS:
        ...
        if statut == 2:
            for site in login_map.get(grp, []):
                tasks.append((site, f"{SERVER_PREFIX}{_safe_id(site)}_mvtart"))
        else:
            tasks.append((grp, f"{SERVER_PREFIX}{_safe_id(grp)}_mvtart"))

    logger.info(f"{len(tasks)} table(s) mvtart à synchroniser")

    # 4. Récupérer les dernières dates en parallèle
    ...
    for identifier, target in tasks:
        ...
        min_date = df["dtemvt"].dropna().min()
        if pd.isna(min_date):
            logger.warning(f"  [{identifier}] dtemvt toutes nulles, skip")
            continue
        ...
        for attempt in range(MAX_WRITE_RETRIES + 1):
            try:
                source.query(
                    ...
                    f"WHERE dtemvt >= TIMESTAMP '{min_date_str} 00:00:00'"
                )
                ...
                if _is_retriable(e) and attempt < MAX_WRITE_RETRIES:
                    ...
                    logger.warning(
                        f"  [{identifier}] erreur transitoire (tentative {attempt+1}/{MAX_WRITE_RETRIES})"
    ...
    if echecs:
        ...
        logger.info("load_mvtart V2 terminé avec succès.")
