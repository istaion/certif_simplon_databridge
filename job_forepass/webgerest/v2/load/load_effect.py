"""
Chargement incrémental de effect en architecture V2.

Pour chaque table cible {PREFIX_TABLE}{safe_id(identifier)}_effect :
  - SELECT MAX(efdate) → from_date (ou DEFAULT_START_DATE si vide)
  - Fetch API avec from_date
  - DELETE WHERE efdate >= min(données reçues)
  - bulk_insert
...
# même si MAX(efdate) est dans le futur (dates de planification, anomalies).
...
def _transform(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df.rename(columns={"efcleunik": "id_effect"})
    ...
    # DATE → pd.Timestamp (strftime utilisé plus bas pour le DELETE)
...
def _get_last_date(source, target_table: str) -> str:
    ...
    try:
        result = source.query(f"SELECT MAX(efdate) AS last_date FROM {target_table}")
        ...
        logger.warning(f"  [{target_table}] impossible de lire MAX(efdate): {e}")
...
def customfunc(event):
    logger.info("job : load_effect V2")
    ...
    for grp in LOGIN_GROUPS:
        ...
        if statut == 2:
            for site in login_map.get(grp, []):
                tasks.append((site, f"{SERVER_PREFIX}{_safe_id(site)}_effect"))
        else:
            tasks.append((grp, f"{SERVER_PREFIX}{_safe_id(grp)}_effect"))

    logger.info(f"{len(tasks)} table(s) effect à synchroniser")
    ...
    for identifier, target in tasks:
        ...
        min_date = df["efdate"].dropna().min()
        if pd.isna(min_date):
            logger.warning(f"  [{identifier}] efdate toutes nulles, skip")
            continue
        ...
        for attempt in range(MAX_WRITE_RETRIES + 1):
            try:
                source.query(
                    f"DELETE FROM {target} WHERE efdate >= DATE '{min_date_str}'"
                )
    ...
    if echecs:
        ...
        logger.info("load_effect V2 terminé avec succès.")
