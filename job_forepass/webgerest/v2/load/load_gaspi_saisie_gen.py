"""
Chargement incrémental de gaspi_saisie_gen en architecture V2.

Pour chaque table cible {PREFIX_TABLE}{safe_id(identifier)}_gaspi_saisie_gen :
  - SELECT MAX(datej) → from_date (ou DEFAULT_START_DATE si vide)
  - Fetch API avec from_date
  - DELETE WHERE datej >= min(données reçues)
  - bulk_insert
...
# même si MAX(datej) est dans le futur (dates de planification, anomalies).
...
_FINAL_COLUMNS = [
    ...
    "cle_jps", "eff_prev", "pas_de_tri", "commentaire", "eff_prod",
    "eff_reel_service",
]
...
def _transform(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in ["id_gaspi_saisie_gen", "eff_prev", "eff_prod", "eff_reel_service"]:
        if col in df.columns:
            df[col] = pd.to_numeric(df[col], errors="coerce").astype("Int64")

    if "qte_gen" in df.columns:
        df["qte_gen"] = pd.to_numeric(df["qte_gen"], errors="coerce").astype("float64")

    # DATE → pd.Timestamp (strftime utilisé pour le DELETE)
    if "datej" in df.columns:
        df["datej"] = pd.to_datetime(df["datej"], errors="coerce")

    if "pas_de_tri" in df.columns:
        df["pas_de_tri"] = df["pas_de_tri"].map(
...
def _fetch(identifier: str, from_date: str) -> pd.DataFrame | None:
    ...
    for attempt in range(3):
        resp = requests.get(
            f"{WEBGEREST_BASE_URL}/gaspi_saisie_gen",
...
def _get_last_date(source, target_table: str) -> str:
    ...
    try:
        result = source.query(f"SELECT MAX(datej) AS last_date FROM {target_table}")
        ...
        logger.warning(f"  [{target_table}] impossible de lire MAX(datej): {e}")
...
def customfunc(event):
    logger.info("job : load_gaspi_saisie_gen V2")
    ...
    for grp in LOGIN_GROUPS:
        ...
        if statut == 2:
            for site in login_map.get(grp, []):
                tasks.append((site, f"{SERVER_PREFIX}{_safe_id(site)}_gaspi_saisie_gen"))
        else:
            tasks.append((grp, f"{SERVER_PREFIX}{_safe_id(grp)}_gaspi_saisie_gen"))

    logger.info(f"{len(tasks)} table(s) gaspi_saisie_gen à synchroniser")
    ...
    for identifier, target in tasks:
        ...
        min_date = df["datej"].dropna().min()
        if pd.isna(min_date):
            logger.warning(f"  [{identifier}] datej toutes nulles, skip")
            continue
        ...
        for attempt in range(MAX_WRITE_RETRIES + 1):
            try:
                source.query(
                    f"DELETE FROM {target} WHERE datej >= DATE '{min_date_str}'"
                )
    ...
    if echecs:
        ...
        logger.info("load_gaspi_saisie_gen V2 terminé avec succès.")
