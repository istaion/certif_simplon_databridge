"""
Lance le job GlobalDepXGB (POST /predict/global_dep_xgb) sur l'API databridge
et attend sa fin en pollant GET /predict/status/{job_id}.

XGBoost entraîné sur tous les établissements départementaux, tous environnements
confondus — pas de paramètre environnement_client.
...
la config Caddy n'est pas résolu :
  ...
  - X-Api-Token : auth applicative databridge, obtenu via POST /auth/token
    avec CLIENT_KEY/CLIENT_SECRET (rôle admin, requis par /predict/global_dep_xgb).
"""
...
def _launch_global_dep_xgb(api_token: str) -> str:
    resp = requests.post(
        f"{BASE_URL}/predict/global_dep_xgb",
        auth=_CADDY_AUTH,
    ...
    if not job_id:
        raise RuntimeError(f"job_id non reçu depuis /predict/global_dep_xgb : {resp.json()}")
    return job_id
...
def _wait_for_job(api_token: str, job_id: str) -> dict:
    ...
    while True:
        ...
        logger.info(f"[predict/global_dep_xgb] job {job_id} — statut : {job_status}")

        if job_status == "completed":
            return job
        if job_status == "failed":
            logger.error(f"[predict/global_dep_xgb] job {job_id} en échec : {job.get('error')}")
            raise RuntimeError(f"Job {job_id} en échec : {job.get('error')}")
        if time.time() > deadline:
            logger.error(
                f"[predict/global_dep_xgb] job {job_id} toujours en statut '{job_status}' après {TIMEOUT_SECONDS}s"
            )
            raise TimeoutError(
...
def customfunc(event):
    ...
    job_id = _launch_global_dep_xgb(api_token)
    logger.info(f"[predict/global_dep_xgb] job lancé : {job_id}")
    job = _wait_for_job(api_token, job_id)
    logger.info(
        f"[predict/global_dep_xgb] job {job_id} terminé avec succès — durée : {job.get('duration_seconds')}s"
    )
