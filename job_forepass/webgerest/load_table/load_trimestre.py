...
table_source = "trimestre"
prefix_table = PARAMS['PREFIX_TABLE']
table_cible = f"{prefix_table}trimestre"
...
def transform_dataframe(df: pd.DataFrame, login_site, descfic_statut) -> pd.DataFrame:
    ...
    df = df[[
        'exercice', 'datdeb', 'datfin', 'nbjtrim', 'notrim'
    ]]

    numerical_columns = ['nbjtrim', 'notrim']
    date_columns = ['datdeb', 'datfin']
    ...
    # Conversion date (format YYYYMMDD)
    ...
    df["pk"] = str(login_site) + "_" + df["notrim"].astype(str) + "_" + df["exercice"].astype(str)
...
def customfunc(event):
    ...
    # Récupération du statut de trimestre par logingroupe depuis descfic
    df_descfic = source.query(
        f"SELECT login_group, statut FROM {prefix_table}descfic WHERE nomfic = 'TRIMESTRE'"
    )
    statut_by_group = {}
    ...
    for row in df_login_site.itertuples():
        ...
        else:from data_process.jobs import run_load_webgerest_v2_all_tables_job
