"""
Full reload des articles — TEMPORAIRE
Remplace load_article.py tant que l'API webgerest n'accepte pas from_date.
Stratégie : DELETE par login_site puis INSERT toutes les données.
"""
...
def get_webgerest_table_data(table_name: str, login_request: str):
...
def fetch_for_login(login: str, descfic_statut=None):
    """Fetch API complet (sans from_date) + transform — pas d'écriture Trino (thread-safe)."""
    try:
        logger.info(f"  [{login}] Fetch API complet (full reload)")
...
def customfunc(event):
    ...
    for row in rows:
        ...
        if statut == 1:
            ...
            login_params.append((login_group, 1))
        else:
            login_params.append((row.login, statut))
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        futures = {
            executor.submit(fetch_for_login, login, descfic_statut): login
            for login, descfic_statut in login_params
        }
