...
table_cible = f"{prefix_table}fitech"
login_table = f"{prefix_table}login"
primary_keys = ['pk']
column_updates = None  # Pas de colonne de mise à jour -> mode bulk_insert
...
# Colonnes de type tableau à sérialiser en JSON string
ARRAY_COLUMNS = [
    'grammages', 'typcond', 'regime', 'menus', 'condtucl',
    'allergenes', 'idlabel',
    'poids_ingredient_principal_calcul',
    'poids_ingredient_principal_saisi'
]
...
def transform_dataframe(df: pd.DataFrame, login_site, descfic_statut) -> pd.DataFrame:
    ...
    # Sérialisation des colonnes tableau en JSON string avant tout autre traitement
    ...
    columns_to_keep = [
        ...
        'codate', 'nbparts', 'datcre', 'datmaj', 'pu', 'fsfcleuniq',
        'lipides', 'glucides', 'protides', 'energie',
        'tprep', 'tcuis', 'trefr', 'tremi', 'conserv', 'derfab', 'dersor',
        'delliv', 'typlib', 'etapes', 'image', 'codftg4', 'clarec',
        'idfrequence', 'idmatos', 'puht', 'usart', 'codtva', 'codftfab',
        'coefpv', 'pvht', 'pvttc', 'sel', 'pcb', 'reglearr', 'tempcons',
        'porc', 'valide', 'dateval', 'idenseignant', 'validedef', 'intolerance',
        'pcbliv', 'statut', 'c0upe', 'sh', 'etat', 'typcondtucl',
        'circuitcourt', 'idobjetft', 'bio', 'vegetarien', 'maison',
        'saison', 'libmenu', 'antigaspi', 'tconserv_min',
        'grammages', 'typcond', 'regime', 'menus', 'condtucl',
        'allergenes', 'idlabel',
    ...
    numerical_columns = ['codft', 'codrec', 'typrec', 'nbparts', 'conserv', 'delliv',
                         'image', 'clarec', 'idfrequence', 'idmatos', 'codftfab',
                         'pcb', 'tempcons', 'pcbliv', 'statut', 'c0upe', 'etat',
                         'typcondtucl', 'idobjetft', 'idenseignant', 'tconserv_min']
    float_columns = ['pu', 'puht', 'pvht', 'pvttc', 'coefpv',
                     'lipides', 'glucides', 'protides', 'energie']
    boolean_columns = ['sel', 'reglearr', 'porc', 'valide', 'validedef',
                       'intolerance', 'sh', 'circuitcourt', 'bio', 'vegetarien',
                       'maison', 'antigaspi']
    ...
    for col in boolean_columns:
        if col in df.columns:
            df[col] = df[col].map(
                lambda x: True if str(x).strip().lower() in ('true', '1') else
                          False if str(x).strip().lower() in ('false', '0') else np.nan
            )

    # Ajout des colonnes métier À LA FIN
    df["login_site"] = str(login_site)
    df["pk"] = str(login_site) + "_" + df["codft"].astype(str)
...
def customfunc(event):
    ...
    # Récupération du statut de fitech par logingroupe depuis descfic
    df_descfic = source.query(
        f"SELECT login_group, statut FROM {prefix_table}descfic WHERE nomfic = 'FITECH'"
    )
    statut_by_group = {}
