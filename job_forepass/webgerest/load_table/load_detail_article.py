...
table_source = "detailarticle"
prefix_table = PARAMS['PREFIX_TABLE']
table_cible = f"{prefix_table}detail_article"
...
column_trino = {
    ...
    "vit_b2": "DOUBLE",
    "cuivre": "DOUBLE",
    "vit_a_retinol": "DOUBLE",
    "vit_k": "DOUBLE",
    "vit_b9": "DOUBLE",
    "potassium": "DOUBLE",
    "phosphore": "DOUBLE",
    "vit_b1": "DOUBLE",
    "vit_a_beta_caro": "DOUBLE",
    "vit_b5": "DOUBLE",
    "acide_gras_sature": "DOUBLE",
    "circuit_court": "BOOLEAN",
    "favori": "BOOLEAN",
    "jamais_local": "BOOLEAN",
    "login_site": "VARCHAR",
...
def transform_dataframe(df: pd.DataFrame, login_site, descfic_statut) -> pd.DataFrame:
    ...
    # Convertir toute colonne contenant des listes/dicts en JSON string
    for col in df.columns:
        if df[col].apply(lambda x: isinstance(x, (list, dict))).any():
            df[col] = df[col].apply(
                lambda x: json.dumps(x) if isinstance(x, (list, dict))
                else (None if (x is None or (isinstance(x, float) and np.isnan(x))) else str(x))
            )

    cols_cible = [
        "login_site", "id_detailarticle", "arcleunik",
        "circuit_court", "saison_deb", "saison_fin", "id_label", "id_origine",
        "codart", "zinc", "fer", "iode", "selenium", "dha", "epa",
        "vit_b3", "vit_b6", "vit_b12", "vit_d", "id_calories",
        "calcium", "sodium", "sucre", "glucose", "amidon", "fibre",
        "vit_c", "magnesium", "vit_e", "vit_b2", "cuivre",
    ...
    df["pk"] = str(login_site) + "_" + df["id_detailarticle"].astype(str)
    df["descfic_statut"] = descfic_statut

    # BIGINT
    for col in ["id_detailarticle", "arcleunik", "saison_deb",
                "saison_fin", "id_label", "id_origine", "codart", "id_calories"]:
        if col in df.columns:
            df[col] = pd.to_numeric(df[col], errors="coerce").astype("Int64")

    # DOUBLE
    for col in ["zinc", "fer", "iode", "selenium", "dha", "epa",
                "vit_b3", "vit_b6", "vit_b12", "vit_d",
                "calcium", "sodium", "sucre", "glucose", "amidon", "fibre",
                "vit_c", "magnesium", "vit_e", "vit_b2", "cuivre",
                "vit_a_retinol", "vit_k", "vit_b9", "potassium", "phosphore",
                "vit_b1", "vit_a_beta_caro", "vit_b5", "acide_gras_sature"]:
        if col in df.columns:
            df[col] = pd.to_numeric(df[col], errors="coerce").astype("float64")

    # BOOLEAN
    for col in ["circuit_court", "favori", "jamais_local"]:
        if col in df.columns:
            df[col] = df[col].map(
...
def customfunc(event):
    ...
    df_descfic = source.query(
        f"SELECT login_group, statut FROM {prefix_table}descfic WHERE UPPER(nomfic) = 'DETAILARTICLE'"
    )
    statut_by_group = {}
