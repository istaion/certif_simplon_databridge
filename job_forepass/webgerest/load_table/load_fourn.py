...
table_cible = f"{prefix_table}fourn"
...
def transform_dataframe(df: pd.DataFrame, login_group, descfic_statut) -> pd.DataFrame:
    ...
    # Renommage manuel : LABEL → id_label (snake_case donne "label", Trino attend "id_label")
    if 'label' in df.columns:
        df.rename(columns={'label': 'id_label'}, inplace=True)

    # Conversion du champ jours (array → JSON string)
    if 'jours' in df.columns:
        df['jours'] = df['jours'].apply(
    ...
    # Sélection des colonnes nécessaires (correspondance Trino)
    df = df[[
        'f_ocleunik', 'codfou', 'libfou', 'codfamfou', 'cpfou',
        'minfac', 'jours', 'delai', 'numcli', 'percde',
        'statut', 'login', 'circuit_court', 'num_agrement', 'bio',
        'no_engagement', 'id_fourn_ext', 'id_origine', 'id_label',
        'tags', 'agrimer_fl', 'agrimer_lait', 'circuit_court_strict'
    ]]

    numerical_columns = [
        'f_ocleunik', 'minfac', 'delai', 'percde', 'statut',
        'id_fourn_ext', 'id_origine', 'id_label'
    ]
    date_columns = []
    ...
    df["login_site"] = str(login_group)
    df["pk"] = str(login_group) + "_" + df["f_ocleunik"].astype(str)
...
def get_last_date_modif_for_login(source, login_group: str) -> str:
    """Récupère la dernière date_modif pour un login_group donné."""
    if column_updates is None:
        return DEFAULT_START_DATE

    try:
        result = source.query(
            f"SELECT MAX({column_updates}) as last_update FROM {table_cible} WHERE login_site = '{login_group}'"
        )
        if isinstance(result, pd.DataFrame) and not result.empty:
        ...
        logger.warning(f"Erreur récupération dernière date pour {login_group}: {e}")
        return DEFAULT_START_DATE


def fetch_and_load_for_login(source, login_request: str, login_group: str, updated_since: str, descfic_statut=None) -> bool:
    """
    ...
    """
    try:
        logger.info(f"  [{login_request}] Récupération depuis: {updated_since}")
        if column_updates is None:
        ...
        if df is not None and not df.empty:
            df_transformed = transform_dataframe(df, login_group, descfic_statut)
            if not df_transformed.empty:
                if column_updates is None:
                    # Mode bulk_insert : supprime les anciennes données du login_group puis insère
                    logger.info(f"  [{login_request}] Mode bulk_insert - Suppression des données existantes...")
                    source.query(f"DELETE FROM {table_cible} WHERE login_site = '{login_group}'")
                    bulk_insert(source, table_cible, df_transformed)
                    logger.info(f"  [{login_request}] {len(df_transformed)} lignes insérées")
                else:
                    # Mode upsert : mise à jour incrémentale
                    upsert(source, table_cible, primary_keys, df_transformed)
                    logger.info(f"  [{login_request}] {len(df_transformed)} lignes upsertées")
            else:
                logger.info(f"  [{login_request}] Aucune donnée après transformation")
        else:
            logger.info(f"  [{login_request}] Aucune donnée depuis {updated_since}")

        return True

    except Exception as e:
        logger.error(f"  [{login_request}] Echec de la récupération: {e}")
        return False
...
def customfunc(event):
    ...
    # Récupération du statut de fourn par logingroupe depuis descfic
    df_descfic = source.query(
        f"SELECT login_group, statut FROM {prefix_table}descfic WHERE nomfic = 'FOURN'"
    )
    statut_by_group = {}
    ...
    for row in df_login_site.itertuples():
        ...
        if statut != 2:
            ...
            success = fetch_and_load_for_login(source, login_group, login_group, DEFAULT_START_DATE, descfic_statut=statut)
            if not success:
            ...
            success = fetch_and_load_for_login(source, row.login, login_group, last_date, descfic_statut=2)
            if not success:
