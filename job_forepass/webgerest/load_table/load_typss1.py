...
table_cible=f"{prefix_table}typss1"
...
def transform_dataframe(df: pd.DataFrame, login_site, descfic_statut) -> pd.DataFrame:
    ...
    df = df[[
        'codss1', 'libss1', 'onilait', 'codcpt', 'boolsh', 'id_catpresta', 'no_variante', 'ordre'
    ]]

    numerical_colunms = [
        'id_catpresta','no_variante','ordre'
    ]
    date_colunms = []
    ...
    df["pk"] = str(login_site) + "_" + df["codss1"].astype(str)
...
def customfunc(event):
    ...
    df_descfic = source.query(
        f"SELECT login_group, statut FROM {prefix_table}descfic WHERE UPPER(nomfic) = 'TYPSS1'"
    )
    statut_by_group = {}
