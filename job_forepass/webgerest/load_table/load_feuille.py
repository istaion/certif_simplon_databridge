...
table_source="feuille"
prefix_table=PARAMS['PREFIX_TABLE']
table_cible=f"{prefix_table}feuille"
...
def transform_dataframe(df: pd.DataFrame, login_site, descfic_statut) -> pd.DataFrame:
    ...
    if not 'id_gaspi_saisie_gen' in df.columns:
        df.rename(columns={'id_gaspi_saisie': 'id_gaspi_saisie_gen'}, inplace=True)

    # Sélection des colonnes nécessaires
    df = df[[
        'fecleunik', 'efdate', 'codss1', 'menu', 'commentaire', 'type_menu', 'id_animation', 'codss2', 'cle_jps', 'nb_vegetarien'
    ]]
    
    numerical_colunms = [
        'nb_vegetarien','fecleunik'
    ]
    date_colunms = ['efdate']
    ...
    df["pk"] = str(login_site) + "_" + df["fecleunik"].astype(str)
...
def customfunc(event):
    ...
    df_descfic = source.query(
        f"SELECT login_group, statut FROM {prefix_table}descfic WHERE UPPER(nomfic) = 'FEUILLE'"
    )
    statut_by_group = {}
