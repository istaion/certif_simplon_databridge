...
table_cible = f"{prefix_table}famart"
...
def transform_dataframe(df: pd.DataFrame, login_site, descfic_statut) -> pd.DataFrame:
    ...
    # Éclater le tableau CPTFAM (10 éléments) en colonnes cptfam_1 à cptfam_10
    if "CPTFAM" in df.columns:
        for i in range(10):
            df[f"cptfam_{i+1}"] = df["CPTFAM"].apply(
                lambda x: x[i] if isinstance(x, list) and len(x) > i else np.nan
            )
        df.drop(columns=["CPTFAM"], inplace=True)
    ...
    cols_cible = [
        "login_site", "codfamart", "libfamart", "typart",
    ...
    # Ajout des colonnes métier
    df["pk"] = str(login_site) + "_" + df["codfamart"].astype(str)
    df["descfic_statut"] = descfic_statut

    # BIGINT
    for col in ["typart", "type", "ordre"]:
        if col in df.columns:
            df[col] = pd.to_numeric(df[col], errors="coerce").astype("Int64")

    # BOOLEAN
    for col in ["statut", "jamais_local"]:
        if col in df.columns:
            df[col] = df[col].map(
...
def customfunc(event):
    ...
    # Récupération du statut de famart par logingroupe depuis descfic
    df_descfic = source.query(
        f"SELECT login_group, statut FROM {prefix_table}descfic WHERE nomfic = 'FAMART'"
    )
    statut_by_group = {}
