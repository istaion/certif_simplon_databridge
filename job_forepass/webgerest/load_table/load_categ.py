...
table_cible=f"{prefix_table}categ"
...
def transform_dataframe(df: pd.DataFrame, login_site, descfic_statut) -> pd.DataFrame:
    ...
    # Sélection des colonnes nécessaires (sans code_site)
    df = df[[
        "codimp","famcat","nbjsem","ordre","regime","statut",
        "typfac","service","codcat","libcat","noncompte","onilait"
    ]]

    # Conversion de certaines colonnes en string
    df["service"] = df["service"].astype(str)
    df["codcat"] = df["codcat"].astype(str)
    ...
    # Conversion explicite des booléens en string lowercase pour compatibilité Trino BOOLEAN
    # (bulk_insert écrit False→'False' avec majuscule, non reconnu par Trino → NULL)
    for col in ['noncompte', 'onilait']:
        if col in df.columns:
    ...
    df["pk"] = str(login_site) + "_" + df["codcat"].astype(str)
...
BOOLEAN_COLS = {'noncompte', 'onilait'}
...
def upsert(source, table: str, primary_keys: list, df: pd.DataFrame):
    ...
    for start in range(0, len(df), step):
        ...
        def col_expr(f):
            return f"CAST(tmp.{f} AS BOOLEAN)" if f in BOOLEAN_COLS else f"tmp.{f}"
        ...
        sql = f"""
            ...
            WHEN MATCHED THEN
                UPDATE SET {','.join([f'{f}={col_expr(f)}' for f in cols if f not in primary_keys])}
            WHEN NOT MATCHED THEN
                INSERT ({','.join(cols)})
                VALUES ({','.join([col_expr(f) for f in cols])})
        """
...
def customfunc(event):
    ...
    # Récupération du statut de categ par logingroupe depuis wg_descfic
    df_descfic = source.query(
        f"SELECT login_group, statut FROM {prefix_table}descfic WHERE nomfic = 'CATEG'"
    )
    statut_by_group = {}
    ...
    for row in df_login_site.itertuples():
        ...
        if statut == 1:
            # Statut 1 = commun au groupe : fetch une seule fois avec logingroupe
            ...
            success = fetch_and_load_for_login(source, login_group, DEFAULT_START_DATE, descfic_statut=1)
            if not success:
                sites_en_echec.append(login_group)
        else:
            # Statut 2 ou inconnu = unique au site : fetch par login_site
