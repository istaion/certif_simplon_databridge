...
table_source = "gaspi_saisie_gen"
prefix_table = PARAMS['PREFIX_TABLE']
table_cible = f"{prefix_table}gaspi_saisie_gen"
...
def transform_dataframe(df: pd.DataFrame, login_site, descfic_statut) -> pd.DataFrame:
    ...
    if 'id_gaspi_saisie_gen' not in df.columns:
        df.rename(columns={'id_gaspi_saisie': 'id_gaspi_saisie_gen'}, inplace=True)

    df = df[[
        'id_gaspi_saisie_gen',
        'datej', 'codss1', 'codss2', 'qte_gen', 'cle_jps',
        'eff_prev', 'pas_de_tri', 'commentaire',
        'eff_prod', 'eff_reel_service'
    ]]

    numerical_columns = [
        'eff_prod', 'eff_reel_service', 'eff_prev', 'id_gaspi_saisie_gen', 'qte_gen'
    ]
    ...
    df["pk"] = str(login_site) + "_" + df["id_gaspi_saisie_gen"].astype(str)
...
def customfunc(event):
    ...
    df_descfic = source.query(
        f"SELECT login_group, statut FROM {prefix_table}descfic WHERE UPPER(nomfic) = 'GASPI_SAISIE_GEN'"
    )
    statut_by_group = {}
