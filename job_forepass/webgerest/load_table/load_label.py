...
table_cible = f"{prefix_table}label"
...
def transform_dataframe(df: pd.DataFrame, login_site, descfic_statut) -> pd.DataFrame:
    ...
    # Renommage : lib_label (de LibLabel) → label (nom colonne Trino)
    df.rename(columns={"lib_label": "label"}, inplace=True)

    df = df.loc[:, ~df.columns.duplicated()]

    cols_cible = [
        "code", "login_site", "id_label", "label", "egalim",
    ]
    ...
    df["pk"] = str(login_site) + "_" + df["id_label"].astype(str)
...
def customfunc(event):
    ...
    # Récupération du statut de label par logingroupe depuis descfic
    df_descfic = source.query(
        f"SELECT login_group, statut FROM {prefix_table}descfic WHERE nomfic = 'LABEL'"
    )
    statut_by_group = {}
