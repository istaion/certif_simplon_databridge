...
table_source = "detpland"
prefix_table = PARAMS['PREFIX_TABLE']
table_cible = f"{prefix_table}detpland"
...
ARRAY_COLUMNS = [
    ...
    'qtemodifr1', 'qtemodifr2', 'qtemodifr3'
]
...
def transform_dataframe(df: pd.DataFrame, login_site, descfic_statut) -> pd.DataFrame:
    ...
    columns_to_keep = [
        'd1_cleunik', 'typrec', 'ordre', 'codrec', 'plcleunik',
        'datfab', 'qtefab', 'codft', 'typord', 'recleuniq',
        'datbes', 'id_mvtpre', 'puht', 'puttc', 'menus',
        'typpart', 'qtemodif', 'qteprev',
        'qtefabr1', 'qtefabr2', 'qtefabr3',
        'qtemodifr1', 'qtemodifr2', 'qtemodifr3',
        'valsor', 'options', 'potage', 'equillibre',
        'horsgamme', 'intolerance', 'no_variante',
        'recleunik_variante', 'taux_prise'
    ]
    columns_to_keep = [c for c in columns_to_keep if c in df.columns]
    df = df[columns_to_keep]

    integer_columns = [
        'd1_cleunik', 'codrec', 'plcleunik', 'codft', 'id_mvtpre',
        'typpart', 'no_variante'
    ]
    float_columns = ['puht', 'puttc', 'taux_prise']
    boolean_columns = [
        'valsor', 'options', 'potage', 'equillibre',
        'horsgamme', 'intolerance'
    ]
    ...
    if 'datfab' in df.columns:
        df['datfab'] = pd.to_datetime(df['datfab'], format='%Y-%m-%d', errors='coerce')
    if 'datbes' in df.columns:
        df['datbes'] = pd.to_datetime(df['datbes'], format='%Y%m%d', errors='coerce')

    df["login_site"] = str(login_site)
    df["pk"] = str(login_site) + "_" + df["d1_cleunik"].astype(str)
...
def customfunc(event):
    ...
    df_descfic = source.query(
        f"SELECT login_group, statut FROM {prefix_table}descfic WHERE nomfic = 'DETPLAND'"
    )
    statut_by_group = {}
    ...
    for row in rows:
        ...
        if statut != 2:
            ...
            login_params.append((login_group, last_date, statut))
        else:
            ...
            try:
                ...
                if pd.notna(min_date):
                    ...
                    source.query(
                        ...
                        f"WHERE login_site = '{login}' "
                        f"AND ({column_updates} >= DATE '{min_date_str}' OR {column_updates} IS NULL)"
                    )
