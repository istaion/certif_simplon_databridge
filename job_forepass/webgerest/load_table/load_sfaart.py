...
table_cible = f"{prefix_table}sfaart"
...
def transform_dataframe(df: pd.DataFrame, login_site, descfic_statut) -> pd.DataFrame:
    ...
    cols_cible = [
        "login_site", "codfamart", "sfaart", "libsfaart",
        "typart", "unite", "type", "statut", "ordre", "jamais_local",
    ]
    ...
    df["pk"] = str(login_site) + "_" + df["codfamart"].astype(str) + "_" + df["sfaart"].astype(str)
...
def customfunc(event):
    ...
    # Récupération du statut de sfaart par logingroupe depuis descfic
    df_descfic = source.query(
        f"SELECT login_group, statut FROM {prefix_table}descfic WHERE nomfic = 'SFAART'"
    )
    statut_by_group = {}
