...
table_cible=f"{prefix_table}typss2"
...
def transform_dataframe(df: pd.DataFrame, login_site, descfic_statut) -> pd.DataFrame:
    ...
    cols_cible = ['codss2', 'libss2', 'onilait', 'codcpt', 'boolsh', 'ordre']
    cols_present = [c for c in cols_cible if c in df.columns]
    df = df[cols_present]

    numerical_colunms = ['ordre']
    date_colunms = []
    ...
    df["pk"] = str(login_site) + "_" + df["codss2"].astype(str)
...
def customfunc(event):
    ...
    df_descfic = source.query(
        f"SELECT login_group, statut FROM {prefix_table}descfic WHERE UPPER(nomfic) = 'TYPSS2'"
    )
    statut_by_group = {}
